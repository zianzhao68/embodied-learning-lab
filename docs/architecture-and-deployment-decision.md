...
### 零成本起步：GitHub Pages（当前不采用）

GitHub Pages 原本作为公开课程的零成本候选：
...
2. 功能完成后提交私有 Git 仓库；合并生产分支后自动发布。
3. 域名准备完成后，将私有仓库接入 EdgeOne Makers。
4. 根据域名备案状态选择加速区域并绑定 HTTPS。
5. 从手机 4G/5G 测试，记录中国移动、联通、电信至少两个网络的首屏时间和可达性。
...
- 已确认仓库暂时不能公开，因此不采用 GitHub Pages 免费公开仓库路径。
